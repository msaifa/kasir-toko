...
import com.komputerkit.kasirtoko.Adapter.AdapterLapPenjualan;
...
public class LPenjualan extends Fragment {
    ...
    public void event(){
        ...
        wExport.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                DDialogExport dialogExport = new DDialogExport(getActivity(),"lpenjualan",sql) ;
    ...
    public void load(){
        ...
        ArrayList<QOrder> list = new ArrayList<>() ;
        AdapterLapPenjualan adapter = new AdapterLapPenjualan(this,list) ;
        recyclerView.setAdapter(adapter) ;


        sql = "select * from qorder where pelanggan like '%"+etCari.getText().toString()+"%' " +
        ...
        if (db.getCount(sql) > 0){
            while(c.moveToNext()){
                ...
                jumlah += utilitas.strToInt(db.getString(c,"totalorder")) ;
            }
        } else {
    ...
    public void print(String faktur) {
        DPrintPreView dPrintPreView = new DPrintPreView(this,getActivity(),faktur,true) ;
        FragmentManager fm = getFragmentManager() ;
