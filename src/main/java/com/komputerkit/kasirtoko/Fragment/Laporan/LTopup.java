...
public class LTopup extends Fragment {
    ...
    public void event(){
        ...
        wExport.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                DDialogExport dialogExport = new DDialogExport(getActivity(),"ltopup",sql) ;
    ...
    public void load(){
        ...
        ArrayList<QDeposit> list = new ArrayList<>() ;
        AdapterLapDeposit adapter = new AdapterLapDeposit(list) ;
        recyclerView.setAdapter(adapter) ;


        sql = "select * from qdeposit where pelanggan like '%"+etCari.getText().toString()+"%' " +
                "and "+myApp.sqlBetweenDate(etDari.getText().toString(),etSampai.getText().toString())+" order by tgldeposit desc"  ;
        Cursor c = db.select(sql) ;

        int jumlah = 0 ;
        if (db.getCount(sql) > 0){
            while(c.moveToNext()){
                list.add(new QDeposit(
                ...
                jumlah += utilitas.strToInt(db.getString(c,"deposit")) ;
            }
        } else {
